...
export async function createOrUpdateProductionCanvas(
  ...
  try {
    const data = await slackApi<{ canvas_id: string }>("conversations.canvases.create", {
    ...
    const canvasId = data.channel?.properties?.canvas?.id;
    if (!data.ok || !canvasId) throw new Error("Existing Slack channel canvas could not be linked");
    await slackApi("canvases.edit", {
      canvas_id: canvasId,
      changes: [{ operation: "replace", document_content: { type: "markdown", markdown } }],
    });
