...
| Super Admin controls — prepared, not yet activated | User-created private `#super-admin` channel plus the existing Slack app’s Interactivity Request URL | `/api/slack/interactions` | Shows the plain-language automation Canvas. The first two tested controls are **Keep Open** for one matching pending archive and **Repair Production Canvas** for one selected channel after an attachment check. The private channel’s membership is the control gate. Setup and one controlled live test are still required before use. |
