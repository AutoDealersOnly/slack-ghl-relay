...
| QR Mail Builder | ADO → Print Shop Tools funnel → QR Dashboard page | ADO custom-menu link to the Print Shop Tools QR page | The preserved original page remains the working foundation. The secret-free append-only add-on supplies QR 1 (`qr_url`), QR 2 (`qr_url_2`), permanent PIN Code, optional KBB Value and Advertised Offer, and separate QR 1/QR 2 settings. Verified September 25, 2026 in **QR Dashboard Clone**; the original step was not edited. See `QR_MAIL_BUILDER_REBUILD.md` and `docs/qr-mail-builder/qr-mail-builder-two-qr-add-on.html`. |
