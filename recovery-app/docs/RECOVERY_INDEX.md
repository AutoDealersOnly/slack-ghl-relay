...
| BDC mailpiece images | Existing **GHL Production Message to Slack** Sent to Print event | `/api/relay/ghl/proof_status` | Posts the original Sent to Print notice first, then converts the selected non-ENV Slack PDFs to two dealership Media-library JPEGs, updates both BDC image custom values, and posts the BDC confirmation. Verified in ABC Dealer. See `BDC_MAILPIECE_IMAGES.md`. |
| Channel creation and scheduled archive | **Create Slack Channel** Custom Webhook plus the daily reconciliation guard | `/api/relay/ghl/create_channel` and `/api/scheduled/relay/archive-reconcile` | Creates or reuses the campaign channel and schedules archive exactly three days after Event End. The daily guard restores missing future schedules and presents overdue channels for approval; it never silently bulk-archives overdue channels. See `AUTOMATIC_ARCHIVE_RECONCILIATION_2026-09-08.md`. |
